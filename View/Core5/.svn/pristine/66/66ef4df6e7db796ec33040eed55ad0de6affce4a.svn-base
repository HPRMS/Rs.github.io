...
.define(function($,movable,resizable,popManager){
    ...
    function resizeDivHtml(objId,kind,width,height,obj){
        var html = [];
        html.push("<div class='s-dragDiv' s-drag='" + objId + "' s-dragStart='dResizeStart' s-dragMove='dResize' s-dragEnd='dResizeEnd'");   //改變當前父控件
        ...
        obj.resizeDivZIndex && html.push(";z-index:" + obj.resizeDivZIndex);
    ...
    function createMixin(pos,width,height){
        var ret = {
            ...
            ,onDResizeStart:function(src,e,addParam){
           ...
           ,onDResize:function(src,e,addParam){
                resize(getResizeDiv(),addParam[0],addParam[1],1,1);
            } 
            
            ,onDResizeEnd:function(src,e,addParam){
