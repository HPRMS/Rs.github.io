...
.define(function(component,$){
    return {
        ...
        //,noShowDelete:true        //directly remove on items
        ,showText:function(){
            if(this.file){
                if(this.file != this._org_file){
                    this.set("text",'<font style="cursor:pointer;color:purple" title="clear select file" s-click="cancelSelect">[Clear]</font>&nbsp;&nbsp;' + this.file);
                }
                else{
                    var url = this.getUrlFromFile(this.file);
                    if(!this.noShowDelete){
                        this.set("text",'<font style="cursor:pointer;color:red" title="delete file" s-click="removeFile">[Delete]</font>&nbsp;&nbsp;<a rel="external" target="_blank" href="' + url.replace("\"","&quot;") + '">' + this.modifyShowText(this.file) + '</a>');
                    }
                    else{
                        this.set("text",'<a rel="external" target="_blank" href="' + url.replace("\"","&quot;") + '">' + this.modifyShowText(this.file) + '</a>');
                    }
                }
            }
            else{
                if(this._org_file){
                    this.set("text",'<font style="cursor:pointer;color:blue" title="cancel delete file" s-click="cancelSelect">[Cancel Delete]</font>&nbsp;&nbsp;<font style="text-decoration:line-through">' + this._org_file + '</font>');
                }
                else{
                    this.set("text","");
                }
        ...
        ,_setFile:function(file,change,oldValue,key,notifyFrom){
            !notifyFrom || ('' + notifyFrom).indexOf("USER_ACT_") != 0 && this.set("_org_file",file);    //only from out set or bind,then set _org_file
            this.isRender() && notifyFrom != "USER_ACT_SELECT_OK" && this.refreshInputFile();           //if not user select ok,clear file
            this.showText();
        }
        ...
        ,onInput:function(){
            ...
            if(val && this._checkType(val)){
                this.set("file",val,"USER_ACT_SELECT_OK");
            }
            else{
