            ...
            for(var bindField in binds){
                ...
                if(key==bindField || (bindField + ".") .indexOf(key + ".")==0){     //完全匹配，直接應用綁定
                    for(var i=0;i<fieldBinds.length;i++){
                        ...
                        //即同一個對象，不能業務數據綁定業務數據
                        //只能view綁定業務數據或UI數據
                        if(!applyedBinds[bind.id] && (!notifyFrom || notifyFrom.kind != "bind" || (notifyFrom.kind=="bind" && target != notifyFrom.source) || bind.bindKind!="set")){
                ...
                //加點防止send_item.purd_items.11與send_item.purd_items.1匹配
                    ...
                    for(var i=0;i<fieldBinds.length;i++){
                        //bindField是哪個?
                        var target = fieldBinds[i].target;
                        if(!notifyFrom || notifyFrom.kind != "bind" || notifyFrom.kind=="bind" && target != notifyFrom.source){
                            var bind = fieldBinds[i];
