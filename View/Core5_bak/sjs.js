...
//        this.setNm = setNm;
    ...
    function getRIAServiceByHost(host){
        var serviceApp = "/RIAService";
        var servicePath = "/ajaxServiceNew.ashx";
        if(host){
            if(host=="/"){
                return servicePath;
            }
            //TODO:目前還不支援https://的service端口
            ...
            if(serviceAppIndex>0){
                ...
                return "http://" + host + serviceApp + servicePath;
            }
        }
    ...
    function _callSjsService(){
        if(_waitingLoad_call_queue.length>0){
            ...
            for(var host in _waitingLoad_call_dic){
                ...
                host = host || _host;
                var jsonpFnName = "_jsonp_" + (host.replace(/\./g,"_").replace(/\//g,"_"));
                ...
                var path = getRIAServiceByHost(host) + "?callback=sjs." + jsonpFnName + "&jsonservice=" + JsonHelper.encode(jsonDic);
                log("調用service",path);        
...
/*
    ...
    function _calSjs(sjsObj,js){
        var needResult = arguments.length == 2;
        var logPrefix = "";
        ...
        if(!needResult || typeof(_sjs_result[sjsResultJs])==="undefined"){
            log(logPrefix,"--計算sjs--",logStack,sjsObj);                
            //檢查找參數是否會死循環
            if(needResult){
                _checkLoopCall(js);
            ...
            for(var i=0;i<usingList.length;i++){
                ...
                if(using.service){
                    ...
                    if(typeof(using_sjsObj)==="object"){
                        var arg = _calSjs(using_sjsObj,using_js);
                        if(!using.noArg){
                            args.push(arg);
                        }
                    }
...
//    }
    ...
    var _sjs_private = window["__sjs__real"]  = window[nm] = {
        ver:"1.3"           //sjs的版本號自動編譯計算在UI.aspx中，然後會在UI.aspx中動態替換代碼，完成UI.aspx的更新，再完成browser的更新
	    ,noConflict: function() {
