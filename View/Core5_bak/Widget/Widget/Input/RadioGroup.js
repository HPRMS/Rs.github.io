...
.define(function(listContainer,toggle,dataFn) {
    return {
        ...
        ,_getItemCfg:function(item,itemIndex){
            ...
            return {
                ...
                ,toggle:this.forceFullEqualSelected ? ((this.valueField?item[this.valueField]:item) === this.selected) : ((this.valueField?item[this.valueField]:item) == this.selected)
            };
        }
        ...
        ,onItemSelect:function(toggle,data){
            var oldValue = this.get("selected");
            this.set("selected",this.valueField?data[this.valueField]:data);
            this.onCommand(this.selected,oldValue);             //用onClick而不是report,可以用配置的方式onClick直接回應事件,而不用report到上面去      
                  
        }
        
        //,evt:""
        
        ,onCommand:function(s,oldValue){
            if(this.evt){
                var item = this._getItemByValue(s);
                item && this.report(this.evt,s,item?item.data:null,item.text,oldValue);
            }
        }
        ...
        ,_getItemByValue:function(value){
            ...
            for(var i=0;i<items.length;i++){
                ...
                if(this.forceFullEqualSelected? itemValue === value :itemValue == value){
                    return items[i];
                }   
            }
            return null;
        ...
        ,focusSelect:function(){
            if(this.isRender() && this.selected){
                var item = this._getItemByValue(this.selected);
                var itemIndex = dataFn.indexOf(this._items,item);
                itemIndex -= 3;
                if(itemIndex<0){
                    itemIndex = 0;
                }
                var showItem = this._items[itemIndex];
                if(showItem){
                    var jq = showItem.jq();
                    jq && jq[0].scrollIntoView(true);
                }
            }
        }
